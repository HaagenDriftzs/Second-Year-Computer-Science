...
FROM Product a 
LEFT OUTER JOIN PrescriptionItem c on a.prescriptionID=c.prescriptionID;
--This finds the all the names of the drug products that have not been used by prescription.
--This is done by using a variables a and c and comparing them using a left outer join.
...
SELECT * 
FROM Product a 
JOIN PrescriptionItem c on a.prescriptionID=c.prescriptionID
WHERE prescribed is not null;
--Is the same as the first but checks if it is not null.
...
FROM Product a 
JOIN PrescriptionItem c on a.prescriptionID=c.prescriptionID
WHERE prescribed is not  = 0;
--This is the same as two except it checks if it is null just without checking for null.
...
FROM Customer
WHERE staffID = '1'
JOIN prescriptionID as 'PrescriptionID' USING prescription
CASE
	WHEN ;
--This checks which prescriptions have been done by Georges pharmacy through checking the staff ID.
...
SELECT custName
FROM Customer
right outer join prescriptionID using (Prescription)
inner join Doctor using (docName);
--This is the combination of a select and a right outer join and an inner join as we are looking for customers
--who do not have a prescription and we also need to output the doctors name.
...
CREATE VIEW CustomerPrescription
SELECT custName, custAddress, custPhone
FROM Customer
right outer join Prescription using (prescriptionID)
inner join Doctor(docName);  
--It works off the same principle of part 5 but I have created a view called CustomerPrescription
--But there is also the addition of some extra columns from customer.
...
--Its the same as part 5 but this time it has a left outer join with the second select embeded within
--the left outer join
...
CREATE VIEW PrescriptionDetail 
SELECT * 
FROM PrescriptionItem
inner join PrescriptionItem using (PrescriptionID, StockDescription, DispensedBy);
-- Created a view called prescription detail uses select and inner join to show the view. 