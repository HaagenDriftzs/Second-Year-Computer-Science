...
FROM Poduct --Selects stockDescription from the product table
UNION --The union shows what is each member of staff has sold or prescribed 
SELECT staffName --Selects the staffName from the staff table
FROM Staff;
...
SELECT staffName
FROM Staff;
--Its the same as the first part, only used an intersect operation this time.
...
SELECT custName, staffName
FROM Customer
inner join Staff using (staffName)
UNION
SELECT stockDescription
FROM Product
WHERE custName, staffName, stockDescription LIKE '%B';
--This first selects the columns needed from the Customer table and staff table using an inner join,
--There is then a union of stockDescription from the product table, but the important part is the
--Where statemnent as we want to find the customer names, staff names and the stock that begins with a capital B.



--4. Write the sql using a SET operation to output the names of doctors that have not appeared on a prescription
-- 20 marks
SELECT docName, docID
FROM Doctor
MINUS
SELECT docID 
FROM Prescription;
--This is simply a minus set operatopm as we want to find the doctors who have not appeared on the prescription table. 
...
SELECT docName, docID
FROM Doctor
inner join prescription using(docID);
--This is similar to part 4 but we want to find what the have appeard on but instead of using a set operation you can use an inner join as it gives the same results.
...
SELECT docName, docID
FROM Doctor
WHERE docID in(select docID from Prescription);
--This is the same as the last two but this time it uses a where statement, as it works the same way, just this 
--time you can compare them through the where statement. 
...
SELECT docName, docID
FROM Doctor
right outer join prescription using(docID);
--Again the same as the last part but this time it is the use of a right outer join.
...
--10 marks
VIEW DoctorLeagueTable
SELECT docName, docID
FROM Doctor
left outer join prescription using(docID);
--This time there is the inclusion of a view called DoctorLeagueTable. This time we want to see doctors name and num prescriptions
--even if they dont appear on it so this time you can use a left outer join to recieve those results.