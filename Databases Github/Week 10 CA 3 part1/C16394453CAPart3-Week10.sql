...
-- 5 marks
ALTER TABLE Product
    ADD markup NUMBER(4,2);--This query alters the product table with the markup coloumn to allow it to accept values up to 99.99
...
-- 5 marks
ALTER TABLE Product
    MODIFY markup NUMBER(4,2) BETWEEN ((retailPrice AND costPrice)/100);
--Alters the product table wtih the markup coloumn to be the difference between the current product retail price and the current product cost price divided by 100.
--This is done with UPDATE, MODIFY, NUMBER to set appropriate values , and BETWEEN which is between retailPrice AND costPrice/100	
	
	
--3. Add a constraint to the markup column so that it cannot contain null
-- 5 marks
ALTER TABLE Product
     MODIFY markup NUMBER(4,2) NOT NULL; --Simply the same as the first part, just added the NOT NULL feature.
...
-- 5 marks
ALTER TABLE Product
     ADD CONSTRAINT drugondrug check 
     IF NULL
     drugondrug=0;
	 --This adds the constraint to check if drugondrug is null and if it is set it to 0. 
...
DELETE product
    WHERE retailPrice;
	--Simply deletes retail price from the Product table.
...
ALTER TABLE product
     ADD CONSTRAINT drugondrug check 
     (drugondrug BETWEEN 0.00 AND 1.00)
     NOT NULL;
     --Only allows drugondrug to be either 0 and 1 and but also that it cant be NULL.
...
UPDATE dosage
FROM product
WHERE stockDescription 'calpol' = 50;
    --Updates the dosage amounts for calpol in the products table to be 50.
...
DELETE FROM prescriptions, prescriptionItems
WHERE customer
    (SELECT custID
     FROM customer
     WHERE (custID) = 1);
	 --Deletes all prescriptions and prescription items for the customer Danny who has a custID of 1.
...
WHERE stockCode = '7'
	...
	SET stockCode = '10'
	WHERE stockCode = '7';
	--This updates all the stockCodes that have umbrella to be changed to the values for the stockCode of LYNX.
	--This was done with selects and a sub query using the update feature
...
WHERE stockCode = '10'
	...
	FROM NonDrugSale
	WHERE stockCode = '10';
	--This Deletes everything from the product and NonDrugSale using sub queries and does it by comparing
	--The lynx stock code.
...
WHERE docID = '2' or '5'
	INSERT Prescription(Notes)
	VALUES('Verify instructions')
WHERE docID = '3'
	INSERT Prescription(Notes)
	VALUES('Check with customer')
WHERE docID = '1' or '4'
	INSERT Prescription(Notes)
	VALUES('Discuss with George');
--Adds a new column to the prescription table called Notes that accepts VARCHAR2 values to 100.
--inserts set values dpending on what docID it is. This is donw using an Alter statement and sub-queries.
